...
| W6 cleanup equivalence (per-step training losses bit-identical before/after, both losses) | CPU proxy ([artifact](artifacts/seqrec-encoder/w6-cleanup-equivalence/)): 2 epochs on a synthetic split, sampled softmax + logQ and gBCE, 18 steps each, **bit-identical** single-threaded (on 64 threads gBCE differs run to run in the last bit for the same code). The GPU 50-step check on yambda-500m d64 and Gate A through the new loader: **pending** (GPU busy) |
