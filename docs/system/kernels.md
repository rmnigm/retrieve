...
Almost every kernel here is Triton. The exceptions are the two
non-Triton SilverTorch backends: a single CUDA C++ translation unit under
[`silvertorch/cuda/`](../../retrieve/src/retrieve/kernels/silvertorch/cuda/),
which backs `SilverTorch(backend="cuda")` and is documented in
[its own section](#codesigned_probe_score_cuda--the-cuda-c-backend), and
its one-to-one CuTe DSL port under
[`silvertorch/cute/`](../../retrieve/src/retrieve/kernels/silvertorch/cute/),
which backs `SilverTorch(backend="cute")`
([its section](#codesigned_probe_score_cute--the-cute-dsl-backend)).
...
- [`silvertorch/`](../../retrieve/src/retrieve/kernels/silvertorch/) —
  ...
  `SilverTorch(backend="cuda")`; `cute/` holds the CuTe DSL port of that
  alternative, selected by `SilverTorch(backend="cute")`.
...
and its CuTe DSL port, which shares its op contract — deliberately
departs from several of them (two launches, no `num_stages`,
`@torch.library.custom_op` instead of `@triton_op`); the cuda section
spells out each divergence.
...
- Tile config (`block_n`, `num_warps`, `num_stages`; `block_p` for the
  silvertorch kernels — the CUDA and CuTe backends' config is
  `(block_p, num_warps, unroll)`, since `num_stages` is a Triton
  pipelining concept)
...
- Graph-break behavior. Every host wrapper in this tree is decorated
  ...
  `codesigned_probe_score_exact_cuda`), and the CuTe DSL backend three
  more with the same signatures (`codesigned_probe_score_cute`,
  `codesigned_probe_score_bloom_cute`,
  `codesigned_probe_score_exact_cute`) — 16 ops across 9 kernel files in
  total (each non-Triton backend's host + device pair counted once).
  `@triton_op` is for Triton
  kernels specifically; a C++ extension or a DSL-compiled callable has
  no `@triton.jit` body for inductor to see, so an opaque custom op is
  the correct registration there. The decorator stops dynamo from
...
under `backend="triton"`; `backend="cuda"` routes the same three
`filter_mode`s to the C++ two-kernel design
...
| C++ | CuTe DSL |
|---|---|
| template parameters `<SEG, HAS_MASK, UNROLL>`, `<C, A>` | `cutlass.Constexpr[int]` / `Constexpr[bool]` kernel arguments; one `cute.compile` per constexpr tuple, memoized in a module dict keyed `("score", SEG, HAS_MASK, UNROLL)`, `("generic", HAS_MASK)`, `("clause", C, A)`, `("bloom",)`, and handed out as a per-device executor (`fn.to(device)`, keyed `(key, device)`) so one compile serves every GPU and a launch never reuses another device's context. `Constexpr` parameters are stripped from the compiled callable's signature — it is called with the runtime arguments plus the stream, in launcher order |
...
- **`seg_reduce_add` is `redux.sync` only**, so the backend is sm_80+;
  the C++ `__shfl_xor_sync` butterfly for older parts is not ported. The
  host module checks `torch.cuda.get_device_capability()` before it
  imports the DSL and reports a pre-Ampere GPU as `CuteMissing` ("cannot
  run here", tests skip), not as a compile failure.
...
aligned, takes `cps_score_kernel_generic`), and ids are trusted. One
check the C++ does not have: a code base pointer that is not even 4-byte
aligned raises an `AssertionError` from `make_ptr` on the host, where the
C++ would issue a misaligned load and fault on the device.
