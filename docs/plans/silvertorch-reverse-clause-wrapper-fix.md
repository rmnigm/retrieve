...
- **Stale oracle disk cache for goodreads-d128/d256 filter** — separate issue, separate remediation (delete cache + rerun; see ⚠ block in [docs/thesis/07-results.md](../thesis/07-results.md)). The wrapper fix here is necessary but not sufficient: even with the wrapper fixed, silvertorch on c1_lang_reverse will still be scored against the stale ground truth unless the oracle is rebuilt.
...
| [docs/thesis/07-results.md](../thesis/07-results.md)                                       | TO EDIT (after rerun) — retire INCOMPATIBLE annotations and the "load-bearing finding" paragraph |

## Sequencing

1. Land the wrapper change (§ Changes 1-3). PR can ship with the new wrapper test passing on a dev GPU; no harness rerun yet.
2. Separately or in the same PR: delete `data/goodreads-work-id/<gt_subdir>/gt_topk_c1_lang_reverse.pt`, `gt_topk_c0c1.pt`, `gt_topk_all4.pt` (and equivalent d64/d256 paths if they exist). This is the goodreads stale-cache remediation from [docs/thesis/07-results.md](../thesis/07-results.md)'s ACTION REQUIRED block; the wrapper fix is necessary but not sufficient — old oracle still poisons recall calculation against silvertorch's now-correct output.
3. Rerun `config/goodreads/d{64,128,256}-filter.yaml` and re-upload to HF.
4. Update [docs/thesis/07-results.md](../thesis/07-results.md) per the "retire methodology caveat" subsection above.
