...
> the campaign is **D1**, and neither has run.
...
| OF-7 | the official and our Triton implementation agree to `jaccard@100` **0.999849 / 0.999805** on goodreads (`n_probe` 24 / 32) but only **0.985033 / 0.984882** on arXiv, where `score_max_abs_diff` is an order of magnitude *smaller* (4.8e-4 vs 5.5e-3) | the shape of a looser filter with more near-ties at the rank-100 boundary. Whether the ≥ 0.99 bar should hold on arXiv is not settled anywhere | [H §12.6](../plans/evaluation-harness-v2.md) (C4). **Unresolved**, carried forward, not explained |
...
| R-3 | the official backend clears `jaccard@100` 0.9998 against our Triton on goodreads and **0.985** on arXiv (OF-7) | not a harness artifact; `score_max_abs_diff` is *smaller* on arXiv | **unresolved** |
...
- **No performance comparison of our Triton kernels against the official ones.**
  ...
  **F2**. Nothing in §3–§5 above may be read as a speed claim.
