...
# 48 epochs x ~370 s = ~4.9 h, patience 10 (restarted; see prediction.md), full val every epoch (~3 % of wall). W&B on, entity pinkmeme.
...
nohup uv run train run \
  ...
  num_epochs=48 patience=10 eval_every=1 eval_batch_size=1024 early_stop_metric=ndcg@10 \
