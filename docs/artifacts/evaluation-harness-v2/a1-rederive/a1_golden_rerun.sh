#!/usr/bin/env bash
# A1 RE-DERIVE — copy of a1_golden_run.sh adjusted for the 2026-09-14 box.
#
# Diff against the original, and only this:
#   * STAGES defaults to `golden` (steps 4-7 stay deferred; the `main`
#     worktree of steps 5/6 does not exist on this box).
#   * every `uv run` that touches the GPU is wrapped in
#     `flock /workspace/gpu.lock` — the GPU is shared with a second worker
#     (CLAUDE.md rule 1). The lock is taken per cell, not for the whole run.
#   * `uv run` -> `uv run --no-sync`, with UV_PROJECT_ENVIRONMENT pointing at
#     /venvs/golden: the venv is built once, up front, and a per-cell re-sync
#     would race the other worker.
#   * NO_CLOCK_LOCK defaults to 1: `nvidia-smi -lgc` is denied in this
#     container (recorded in the A1 record already). The 30 s clock sampler
#     still runs — H §7's fallback.
#
...
#                       docs/artifacts/evaluation-harness-v2/a1)
...
# one level deeper than the original (a1-rederive/), so four parents.
REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}"
EVAL_DIR="$REPO_ROOT/evaluation"
GOLDEN_DIR="${GOLDEN_DIR:-$EVAL_DIR/golden}"
MAIN_GOLDEN_DIR="$GOLDEN_DIR/_main"
LOG_DIR="$GOLDEN_DIR/_logs"
A1_DIR="${A1_DIR:-$REPO_ROOT/docs/artifacts/evaluation-harness-v2/a1}"
...
# A private inductor cache: the default /tmp/torchinductor_root is shared with
# the other worker on this box, and H's C4 finding is that it does not
# invalidate on a @triton_op wrapper source change. Shared across the 11 cells
# (they are 11 processes), isolated from anyone else.
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-$REPO_ROOT/.inductor-cache}"
: "${NO_CLOCK_LOCK:=1}"
MAIN_EVAL_DIR="$MAIN_WORKTREE/evaluation"

STAGES="${STAGES:-golden}"
...
run_cell() {
  ...
  ( cd "$eval_dir" && flock "$GPU_LOCK" uv run --no-sync evaluate \
      --config "$config" \
      --algo "$algo" \
      --backend "$backend" \
      --filter-kind clause \
...
_step4_side() {
  ...
  echo "[run ] step4 ${side}"
  ( cd "$eval_dir" && TORCH_LOGS=graph_breaks flock "$GPU_LOCK" uv run --no-sync evaluate \
...
stage_step6() {
  ...
  python3 "$REPO_ROOT/docs/artifacts/evaluation-harness-v2/a1_step6_diff.py" \
...
stage_step5() {
  ...
  if require_main_worktree; then
    ...
    python3 "$REPO_ROOT/docs/artifacts/evaluation-harness-v2/a1_step5_compare.py" \
...

{
  ...
  ( cd "$EVAL_DIR" && uv run --no-sync python -c \
      "import torch, triton; print('torch', torch.__version__); print('triton', triton.__version__)" 2>&1 )
  echo "silvertorch:"
  ( cd "$EVAL_DIR" && uv run --no-sync python -c \
      "import silvertorch; print(silvertorch.__file__)" 2>&1 )
  echo "library tree: $(git -C "$REPO_ROOT" rev-parse HEAD:retrieve)"
  echo "data root: $(readlink -f "$EVAL_DIR/data")"
...
for f in "$GOLDEN_DIR"/*.json; do
  [ -e "$f" ] || continue
  n=$( (cd "$EVAL_DIR" && uv run --no-sync python -c \
...
echo "_main/, which is gitignored) and docs/artifacts/evaluation-harness-v2/a1/,"
