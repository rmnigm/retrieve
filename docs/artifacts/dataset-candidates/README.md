...
Raw material behind ../dataset-candidates.md
and the Phase E steps of ../00-roadmap.md. One
...
../dataset-candidates.md §7.1.

| file | what it is |
|---|---|
| [prep_log-2026-09-16.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/prep_log-2026-09-16.json) | `yfcc prep` + `yfcc attrs` re-run from a fresh download. Every number equals the 2026-09-06 `prep_log.json` below except the wall clocks (prep 35.7 s, attrs 796 s — the 40× slower attrs was the `/venvs/retrieve` console-script shebang importing numpy over the network volume, see §7.1). |
| [text_emb.meta-2026-09-16.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/text_emb.meta-2026-09-16.json) | The new `*.meta.json` sidecar: `prefix: null` (the declared prefix-free encoder of `layout.prefix_problem`) plus the provenance the old `emb_provenance.json` carried. `bench check --dataset yfcc10m` → `yfcc10m d192: ok`. |
| [filter-yfcc10m-d192-cpu-rehearsal.jsonl](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/filter-yfcc10m-d192-cpu-rehearsal.jsonl) | The `filter` suite's `linr_v1_filter_mask / torch / clause / tags_and` cell run **on the CPU** (`CUDA_VISIBLE_DEVICES=""`, 16 threads, `users_limit` 1000, `--skip-perf --mode eager`) because the D1-a campaign holds the GPU lock for its whole stage. **`status: failed` at the exact-algo quality gate: `recall_oracle@1000 = 0.9641 < 0.99`** — see the diagnostic below and §7.1 item 4. `env.gpu = "cpu"`. |
| [none_e1-yfcc10m-d192-cpu-rehearsal.jsonl](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/none_e1-yfcc10m-d192-cpu-rehearsal.jsonl) | The matching `none` cell from a scratch suite (`none_e1`, not in `config/suites.yaml`, whose unfiltered suite was retired on 2026-09-16): `status: partial` (`skip_perf`, `modes`), held-out recall@100/500/1000 = 0.998, 1,952 s on the CPU. Same caveats. |
...
../dataset-candidates.md §7.2.

| file | what it is |
|---|---|
| [plan-full-2026-09-16.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/pubmed/plan-full-2026-09-16.json) | `eval-data pubmed plan --medline --prefetch 1 --mbps 23.9`: per-shard sizes from `HEAD` (114 MedCPT files + 1334 MEDLINE files) and rows from each npy header — 35,920,666 articles, 217.9 GB to download, peak disk 68.6 GB, 2.54 h at the measured single-stream rate; 110.3 GB of fp32 items on the device. |
| [plan-10m-2026-09-16.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/pubmed/plan-10m-2026-09-16.json) | The same with `--keep-items 10000000`: peak disk 27.2 GB, the same download, 30.7 GB of items on the device. |
...
| [dryrun-chunk37-prep_log.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/pubmed/dryrun-chunk37-prep_log.json), [dryrun-chunk37-shard_index.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/pubmed/dryrun-chunk37-shard_index.json), [dryrun-chunk37-text_emb.meta.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/pubmed/dryrun-chunk37-text_emb.meta.json) | What the dry run wrote: `pmid_ranges_disjoint: true`, one output shard `[0, 380761)`, `prefix: null`. |
...
| [raw-manifest.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/raw-manifest.json) | `yfcc convert --sha256`: sha256 and size of each raw file, plus the header facts parsed out of them — 10,000,000 × 192 uint8 base vectors, 100,000 queries, 108,210,476 item-tag entries over a 200,386-tag vocabulary, filtered GT at k=10, unfiltered GT at k=100. This is the provenance record for **F3**. |
| [prep_log.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/prep_log.json) | `yfcc prep` + `yfcc attrs`. Notable: 10.821 tags/item mean with a 1,517 max; 61,626 single-tag and 38,374 two-tag queries; only 7,910 of 200,386 tags are ever queried; base-vector norms have cv = 0.011. `attrs.gt_fidelity` is what the K=32 tag cap costs — 74,214 of 100,000 queries keep their whole shipped-GT row. |
| [gt_check-cuda-100k.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/gt_check-cuda-100k.json) | **E1's gate on the A100, 2026-09-06**: `yfcc_check_gt --device cuda` over all 100,000 queries — 100,000 id-exact, 0 tie-equivalent, 0 mismatched, `max_abs_distance_error = 0.0`, 1276 s. |
| [gt_check-cpu-200.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/gt_check-cpu-200.json) | `yfcc_check_gt --device cpu --limit 200`, the CPU rehearsal of **E1's gate**: 200/200 queries reproduced, 195 id-exact and 5 tie-equivalent, `max_abs_distance_error = 0.0`. The gate itself is the same command with `--device cuda` and no `--limit`, and is still pending. |
| [gt_check-cpu-100-cosine.json](https://huggingface.co/datasets/pinkmeme/eval-results/blob/main/artifacts/dataset-candidates/yfcc10m/gt_check-cpu-100-cosine.json) | `--metric ip` on 100 queries — a diagnostic, not a gate. An exact *cosine* filtered top-10 (the metric the harness actually scores) reproduces the shipped squared-L2 GT on 59 % of queries at mean recall 0.951. This is the size of deviation 1 in [../../system/datasets.md](../../system/datasets.md#yfcc10m). |
