...
@triton.autotune(configs=_autotune_configs(), key=["P", "D", "W", "HAS_QB"])
@triton.jit
...
def codesigned_probe_score(
    ...
    *,
    ...
    test ``(qb & sig) == qb`` against the item's bloom signature, and (if it
    passes) score ``(item_codes[id].float() @ q) * item_scales[id]``. Items
    failing the filter or with id == -1 (cluster padding) get score ``-inf``.
