"""CPU-only tests for ``bench.oracle`` (blob v4).
...
rate, and ``code_version`` (git tree hash, or the ``files:`` fallback outside git). Plain
torch ops only; no data files, no GPU.
"""
...
from bench.oracle import compute, load_or_build
