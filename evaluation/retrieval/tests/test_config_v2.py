...
Fixture side (``tests/data/v2/``): job counts and keys per suite, the PATHS collapse and
...
from retrieval.algos_v2 import PATHS
from retrieval.config import NONE_SWEEP, ConfigError, load_dataset, load_eval_config, load_matrix

FIX = Path(__file__).parent / "data" / "v2"
...
# ----- the real configs against the old YAMLs -----------------------------------------


def _old_cells(path: Path, filter_kind_sweeps: bool) -> set[tuple]:
    """(algo, backend, filter_kind, sweep, k, bs) of the old harness's config."""
    cfg = load_eval_config(path)
    kinds = (
        [(k, [s.name for s in f.sweeps]) for k, f in cfg.filters.items()]
        if filter_kind_sweeps
        else [("none", [NONE_SWEEP])]
    )
    return {
        (a, b, fk, sw, k, bs)
        for a in cfg.algorithms
        for b in cfg.backends
        for fk, sweeps in kinds
        for sw in sweeps
        for k in cfg.ks
        for bs in cfg.batch_sizes
        if not (a == "linr_v2" and fk == "none")
    }
...
def test_filter_suite_matches_old_d128_filter_config(dataset):
    ...
    assert _new_cells(jobs) == _old_cells(CFG / dataset / "d128-filter.yaml", True)
    # Every filter cell has the same users_limit and paths the old config named.
    old = load_eval_config(CFG / dataset / "d128-filter.yaml")
    ds = jobs[0].data
    assert ds.users_limit == old.users_limit == 10000
    assert str(ds.attrs) == old.filters["clause"].attrs_path
    assert str(ds.reverse) == old.filters["clause"].reverse_path
    assert str(ds.data_dir) == old.data_dir
    assert ds.gt_dir == Path(old.data_dir) / old.gt_subdir
    assert (str(ds.checkpoint) if ds.checkpoint else None) == old.checkpoint
    if ds.content_dir:
        assert ds.content_dir == Path(old.data_dir) / old.content_subdir
...
def test_quality_suite_matches_old_d128_quality_config_modulo_collapse(dataset):
    ...
    old = _old_cells(CFG / dataset / "d128-quality.yaml", False)
    # Old rows whose backend runs the same path as an earlier backend are the collapsed ones.
    old_cfg = load_eval_config(CFG / dataset / "d128-quality.yaml")
    first = {}
    keep = set()
    for a in old_cfg.algorithms:
        for b in old_cfg.backends:
            p = PATHS[(a, "none", b)]
            if p not in first.setdefault(a, {}):
                first[a][p] = b
                keep.add((a, b))
    assert _new_cells(jobs) == {c for c in old if (c[0], c[1]) in keep}
    assert (
        len(jobs) == 1 + 2 + 1 + 2
    )  # linr_v1, linr_v3 × 2, linr_v4, silvertorch × {triton, torch}
