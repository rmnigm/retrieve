...
Each entry returns a ``(forward, modules)`` pair: ``forward(query) -> (ids, scores)``
and the list of underlying ``nn.Module``\\s holding GPU buffers (so callers can
``del`` them between cells to release memory).
...
) -> tuple[ForwardFn, list[torch.nn.Module]]:
